...
class AppStore extends Component {
  ...
  getFilteredSearchedApp = () => {
    const {searchInput} = this.state
    const filteredProjects = this.getFilteredProjects()
    const filteredProjectsName = filteredProjects.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return searchInput === '' ? filteredProjects : filteredProjectsName
  }

  render() {
    const filteredProjects = this.getFilteredSearchedApp()
    const {activeTabID} = this.state
    const {searchInput} = this.state
    return (
      <div className="bg-container">
        <h1 className="App-heading"> App Store bolthey</h1>
